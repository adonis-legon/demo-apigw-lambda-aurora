resource "aws_iam_role" "demo_apigw_lambda_aurora_order_function_role" {
  name = "${var.environment}-demo-apigw-lambda-order-function-role"
  ...
  inline_policy {
    name = "${var.environment}-demo-apigw-lambda-order-function-policy"
...
resource "aws_iam_role_policy_attachment" "demo_apigw_lambda_aurora_order_function_role_vpc_access_execution" {
    role       = aws_iam_role.demo_apigw_lambda_aurora_order_function_role.name
    policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaVPCAccessExecutionRole"
}

resource "aws_iam_role_policy_attachment" "demo_apigw_lambda_aurora_order_function_role_xray_write_only_access" {
    role       = aws_iam_role.demo_apigw_lambda_aurora_order_function_role.name
...
resource "aws_lambda_function" "demo_apigw_lambda_aurora_order_function" {
  ...
  role           = aws_iam_role.demo_apigw_lambda_aurora_order_function_role.arn
